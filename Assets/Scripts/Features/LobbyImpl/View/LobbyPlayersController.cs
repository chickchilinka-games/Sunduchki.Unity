...
{
    public class LobbyPlayersController : MonoBehaviour, IDisposable
    {
        ...
        [Inject]
        public void Construct(LobbyService lobbyService, PlayerRosterService rosterService)
        {
        ...
        {
            var localId = FindPlayerId(players, true);
            var opponentId = FindPlayerId(players, false);

            _localPlayerView?.BindPlayerId(localId);
            _opponentPlayerView?.BindPlayerId(opponentId);

            if (!string.IsNullOrWhiteSpace(localId))
            {
                _rosterService.SetLocalPlayer(localId);
                EnsureRosterInfo(localId).Forget();
            }

            if (!string.IsNullOrWhiteSpace(opponentId))
            {
                EnsureRosterInfo(opponentId).Forget();
            }
        ...
        private async UniTask EnsureRosterInfo(string playerId)
        {
            if (_rosterService.TryGetPlayer(playerId, out _))
            {
                return;
            }

            await _rosterService.FetchPlayerAsync(playerId);
        }
