...
## Version 3.0 was published ##

* You should get access to [the Vault](https://vault.chickchilinka.xyz/ui/vault/dashboard) by #helpdesk
    ** In the editor you can authorize by LDAP
* Before build you should make sure your project's key files, ids, tokens and etc are added to [the Vault](https://vault.chickchilinka.xyz/ui/vault/dashboard) for each environments.
* You should receive that values and set they up in the ApplyEnvironment method in the ICICustomConfig implementation in your project. 
* Remove all secret values from your project, include "magic string", configs, keystore files, firebase files and etc.

## Installation

This package will be published to NPM registry.  To use:

1. Make sure you've added Chickchilinka scoped registry to Unity. [Here is how](https://docs.chickchilinka.io/en/General/General/%F0%9F%92%A1+How-to+articles/Working+with+internal+NPM+repository)
2. Add dependency to the `Packages/manifest.json' file:

   ```
   "io.chickchilinka.tools.cicd": "3.0.12"
   ```

   where 3.0.12 is the package version
...
2. *Pre-build* platform-dependant functions are called
	  ...
	  * For Android the signing options are set from the Environment variables, see [here](https://git.chickchilinka.io/chickchilinka/general/templates/-/blob/master/unity-flow/README.md#android-signing)
...
```json
{
   ...
   "appId": "com.Chickchilinka.io.chickchilinka.tools.cicd",
   "version": "1.1b1",
   "environment": "TEST",
